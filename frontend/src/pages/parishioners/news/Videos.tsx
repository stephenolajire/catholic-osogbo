...
import { Play, Pause, ChevronRight, Film } from "lucide-react";
import { useVideos } from "../../../hooks/news/useVideo";

const Videos = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const [playingId, setPlayingId] = useState<string | null>(null);
  const { data, isLoading, isError } = useVideos(currentPage, 12);
    ...
    <section className="py-20 px-6 md:px-16 lg:px-24 bg-neutral-50">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-start gap-5">
            ...
            <div>
              ...
              <p className="text-neutral-600 text-lg leading-relaxed font-light">
                Watch videos, Mass celebrations, and diocesan content
              </p>
            </div>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            <p className="text-neutral-600 text-lg">No videos available.</p>
          </div>
        )}
      </div>
    </section>
