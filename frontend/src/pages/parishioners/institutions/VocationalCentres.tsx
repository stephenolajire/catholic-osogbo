...
const VocationalCentres = () => {
  const [selectedSubcategory, setSelectedSubcategory] =
    useState<InstitutionSubcategory | null>(null);

  const { data: category, isLoading } = useInstitutionCategory("vocational");
  const displayCategory =
    category ||
    DEFAULT_INSTITUTION_CATEGORIES.find((c) => c.categoryKey === "vocational");
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* Hero Section */}
      <InstitutionHero
        title={selectedSubcategory?.name || "Vocational Training Centres"}
        description={
          selectedSubcategory?.description ||
          "Skills training for empowerment and livelihood. Our vocational institutions teach practical skills and crafts to equip individuals for sustainable employment and economic self-reliance."
        }
        totalInstitutions={
          ...
          {isLoading ? (
            ...
            <div>
              ...
              <p className="text-amber-700/70 mb-12">
                Choose a training programme to view available institutions
              </p>
...
export default VocationalCentres;
