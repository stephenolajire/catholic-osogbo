...
const HealthCare = () => {
  ...
  const { data: category, isLoading } = useInstitutionCategory("healthcare");
  const displayCategory =
    category ||
    DEFAULT_INSTITUTION_CATEGORIES.find((c) => c.categoryKey === "healthcare");

  const totalInstitutions =
    displayCategory?.subcategories?.reduce(
      (sum, sub) => sum + (sub.institutions?.length || 0),
      0,
    ) || 0;
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* Hero Section */}
      <InstitutionHero
        title={selectedSubcategory?.name || "Healthcare Institutions"}
        description={
          selectedSubcategory?.description ||
          "Compassionate medical care guided by Catholic values. Our healthcare facilities provide comprehensive services combining modern medical expertise with spiritual care and community health outreach."
        }
        totalInstitutions={
          ...
          {isLoading ? (
            ...
            <div>
              ...
              <p className="text-amber-700/70 mb-12">
                Choose a healthcare facility type to view available institutions
              </p>
